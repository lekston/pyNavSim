...
plt.xlabel("Time [s]")
plt.ylabel(r"$p\left[\frac{rad}{s}\right]$")
plt.legend([r"$p$", r"$\dot{p}_{cmd}$"])
plt.grid()
...
plt.xlabel("Time [s]")
plt.legend([r"$V_{E}$ [$m/s$]", r"$XtrackErr$ [$m$]", r"$L1_{dist}$"])
plt.grid()
...
plt.plot(time, [bn.wrap_180(np.rad2deg(psi)) for psi in sim.logs['S_psi']],
         time, gnd_track)
plt.xlabel("Time [s]")
plt.legend([r"$\psi$ [$^\circ$]", r"$\psi_{gnd}$ [$^\circ$]"])
plt.grid()

fig5 = plt.figure()
plt.plot(time, sim.logs['L1_wca_in'], time, sim.logs['L1_wca_out'])
plt.xlabel("Time [s]")
plt.legend([r"$wca_{in}$ [$rad$]", r"$wca_{out}$ [$rad$]"])
plt.grid()
